...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'insert') {
    ...
    switch ($table) {
        ...
        case 'customer':
            ...
            break;
        case 'vehicle_type': 
            $stmt = $link->prepare("INSERT INTO vehicle_type (vehicletype_name, vehicletype_multiplier) VALUES (?, ?)");
            $stmt->bind_param("sd", $_POST['vehicletype_name'], $_POST['vehicletype_multiplier']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'service_type': 
            $stmt = $link->prepare("INSERT INTO service_type (serviceType_Name, serviceType_BasePrice) VALUES (?, ?)");
            $stmt->bind_param("sd", $_POST['serviceType_Name'], $_POST['serviceType_BasePrice']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'employee_position': 
            $stmt = $link->prepare("INSERT INTO employee_position (pos_name, pos_salary) VALUES (?, ?)");
            $stmt->bind_param("sd", $_POST['pos_name'], $_POST['pos_salary']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'employee': 
            $stmt = $link->prepare("INSERT INTO employee (emp_fname, emp_lname, emp_address, emp_username, emp_password, branch_ID, pos_ID) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssii", $_POST['emp_fname'], $_POST['emp_lname'], $_POST['emp_address'], $_POST['emp_username'], $_POST['emp_password'], $_POST['branch_ID'], $_POST['pos_ID']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'membership':
            $stmt = $link->prepare("INSERT INTO membership (membership_name, membership_description, membership_point, cust_ID) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssii", $_POST['membership_name'], $_POST['membership_description'], $_POST['membership_point'], $_POST['cust_ID']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'vehicle':
            $stmt = $link->prepare("INSERT INTO vehicle (vehicle_plate, vehicle_color, cust_ID, vehicletype_ID) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssii", $_POST['vehicle_plate'], $_POST['vehicle_color'], $_POST['cust_ID'], $_POST['vehicletype_ID']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'booking':
            $stmt = $link->prepare("INSERT INTO booking (booking_date, booking_status, cust_ID, branch_ID) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssii", $_POST['booking_date'], $_POST['booking_status'], $_POST['cust_ID'], $_POST['branch_ID']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
        case 'payment':
        ...
        case 'receipt':
            ...
            break;
        case 'service_detail': 
            $stmt = $link->prepare("INSERT INTO service_detail (sdetail_quantity, sdetail_price, service_ID, serviceType_ID, emp_ID) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("idiii", $_POST['sdetail_quantity'], $_POST['sdetail_price'], $_POST['service_ID'], $_POST['serviceType_ID'], $_POST['emp_ID']);
            $stmt->execute() ? print("เพิ่มสำเร็จ") : print("" . $stmt->error);
            break;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'update') {
    ...
    switch ($table) {
        ...
        case 'customer':
            ...
            break;
        case 'vehicle_type': 
            $stmt = $link->prepare("UPDATE vehicle_type SET vehicletype_name=?, vehicletype_multiplier=? WHERE vehicletype_ID=?");
            $stmt->bind_param("sdi", $_POST['vehicletype_name'], $_POST['vehicletype_multiplier'], $id);
            break;
        case 'service_type': 
            $stmt = $link->prepare("UPDATE service_type SET serviceType_Name=?, serviceType_BasePrice=? WHERE serviceType_ID=?");
            $stmt->bind_param("sdi", $_POST['serviceType_Name'], $_POST['serviceType_BasePrice'], $id);
            break;
        case 'employee_position': 
            $stmt = $link->prepare("UPDATE employee_position SET pos_name=?, pos_salary=? WHERE pos_ID=?");
            $stmt->bind_param("sdi", $_POST['pos_name'], $_POST['pos_salary'], $id);
            break;
        case 'employee': 
            $stmt = $link->prepare("UPDATE employee SET emp_fname=?, emp_lname=?, emp_address=?, emp_username=?, emp_password=?, branch_ID=?, pos_ID=? WHERE emp_ID=?");
            $stmt->bind_param("sssssiii", $_POST['emp_fname'], $_POST['emp_lname'], $_POST['emp_address'], $_POST['emp_username'], $_POST['emp_password'], $_POST['branch_ID'], $_POST['pos_ID'], $id);
            break;
        case 'membership': 
            $stmt = $link->prepare("UPDATE membership SET membership_name=?, membership_description=?, membership_point=?, cust_ID=? WHERE membership_ID=?");
            $stmt->bind_param("ssiii", $_POST['membership_name'], $_POST['membership_description'], $_POST['membership_point'], $_POST['cust_ID'], $id);
            break;
        case 'vehicle': 
            $stmt = $link->prepare("UPDATE vehicle SET vehicle_plate=?, vehicle_color=?, cust_ID=?, vehicletype_ID=? WHERE vehicle_ID=?");
            $stmt->bind_param("ssiii", $_POST['vehicle_plate'], $_POST['vehicle_color'], $_POST['cust_ID'], $_POST['vehicletype_ID'], $id);
            break;
        case 'booking': 
            $stmt = $link->prepare("UPDATE booking SET booking_date=?, booking_status=?, cust_ID=?, branch_ID=? WHERE booking_ID=?");
            $stmt->bind_param("ssiii", $_POST['booking_date'], $_POST['booking_status'], $_POST['cust_ID'], $_POST['branch_ID'], $id);
            break;
        case 'payment':
        ...
        case 'receipt':
            ...
            break;
        case 'service_detail': 
            $stmt = $link->prepare("UPDATE service_detail SET sdetail_quantity=?, sdetail_price=?, service_ID=?, serviceType_ID=?, emp_ID=? WHERE sdetail_ID=?");
            $stmt->bind_param("idiiii", $_POST['sdetail_quantity'], $_POST['sdetail_price'], $_POST['service_ID'], $_POST['serviceType_ID'], $_POST['emp_ID'], $id);
            break;
    }
...
if (isset($_GET['delete']) && isset($_GET['table'])) {
    ...
    $id_field = match($table) {
        'branch' => 'branch_ID',
        'customer' => 'cust_ID',
        'vehicle_type' => 'vehicletype_ID',
        'service_type' => 'serviceType_ID', 
        'employee_position' => 'pos_ID', 
        'employee' => 'emp_ID',
...
<form method="get">
    <select name="table" onchange="this.form.submit()">
        ...
        <option value="employee_position" <?=$t=='employee_position'?'selected':''?>>employee_position</option>
...
<form method="post">
    ...
    switch($t){
        ...
        case 'customer':
            ...
            break;
        case 'vehicle_type':
            echo 'ชื่อประเภท: <input name="vehicletype_name" value="'.($edit_data['vehicletype_name']??'').'" required> ตัวคูณ: <input name="vehicletype_multiplier" type="number" step="0.01" value="'.($edit_data['vehicletype_multiplier']??'').'" required>';
            break;
        case 'service_type': 
            echo 'ชื่อบริการ: <input name="serviceType_Name" value="'.($edit_data['serviceType_Name']??'').'" required> ราคา: <input name="serviceType_BasePrice" type="number" step="0.01" value="'.($edit_data['serviceType_BasePrice']??'').'" required>';
            break;
        case 'employee_position': 
            echo 'ตำแหน่ง: <input name="pos_name" value="'.($edit_data['pos_name']??'').'" required> เงินเดือน: <input name="pos_salary" type="number" step="0.01" value="'.($edit_data['pos_salary']??'').'">';
            break;
        case 'employee': 
            ...
            echo '</select> ตำแหน่ง: <select name="pos_ID"><option value="">-</option>';
            $r=$link->query("SELECT pos_ID,pos_name FROM employee_position");
            while($row=$r->fetch_assoc())echo"<option value='{$row['pos_ID']}' ".(($edit_data['pos_ID']??'')==$row['pos_ID']?'selected':'').">{$row['pos_name']}</option>";
            echo '</select>';
            break;
        case 'membership': 
            ...
            echo ' ลูกค้า: <select name="cust_ID" required><option value="">-</option>';
            $r=$link->query("SELECT cust_ID,cust_fname,cust_lname FROM customer");
            while($row=$r->fetch_assoc())echo"<option value='{$row['cust_ID']}' ".(($edit_data['cust_ID']??'')==$row['cust_ID']?'selected':'').">{$row['cust_fname']} {$row['cust_lname']}</option>";
            echo '</select>';
            break;
        case 'vehicle': 
            ...
            $r=$link->query("SELECT cust_ID,cust_fname,cust_lname FROM customer");
            while($row=$r->fetch_assoc())echo"<option value='{$row['cust_ID']}' ".(($edit_data['cust_ID']??'')==$row['cust_ID']?'selected':'').">{$row['cust_fname']} {$row['cust_lname']}</option>";
            echo '</select> ประเภทรถ: <select name="vehicletype_ID" required><option value="">-</option>';
            $r=$link->query("SELECT vehicletype_ID,vehicletype_name FROM vehicle_type");
            while($row=$r->fetch_assoc())echo"<option value='{$row['vehicletype_ID']}' ".(($edit_data['vehicletype_ID']??'')==$row['vehicletype_ID']?'selected':'').">{$row['vehicletype_name']}</option>";
            echo '</select>';
            break;
        case 'booking': 
        ...
        case 'service_detail': 
            ...
            echo '</select> ประเภทบริการ: <select name="serviceType_ID" required><option value="">-</option>';
            $r=$link->query("SELECT serviceType_ID,serviceType_Name FROM service_type");
            while($row=$r->fetch_assoc())echo"<option value='{$row['serviceType_ID']}' ".(($edit_data['serviceType_ID']??'')==$row['serviceType_ID']?'selected':'').">{$row['serviceType_Name']}</option>";
            echo '</select> พนักงาน: <select name="emp_ID" required><option value="">-</option>';
            $r=$link->query("SELECT emp_ID,emp_fname,emp_lname FROM employee");
            while($row=$r->fetch_assoc())echo"<option value='{$row['emp_ID']}' ".(($edit_data['emp_ID']??'')==$row['emp_ID']?'selected':'').">{$row['emp_fname']} {$row['emp_lname']}</option>";
            echo '</select>';
            break;
    }
    ?>
